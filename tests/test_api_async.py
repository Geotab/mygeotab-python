...
class TestAsyncCallApi:
    ...
    def test_bad_parameters(self, async_populated_api):
        with pytest.raises(MyGeotabException) as excinfo:
            run(
                async_populated_api.call_async("NonExistentMethod", not_a_property="abc123"),
    ...
    def test_add_edit_remove(self, async_populated_api_entity):
        def get_trailer():
            trailers = run(
                async_populated_api_entity.get_async("Trailer", name=TRAILER_NAME), loop=async_populated_api_entity.loop
            )
